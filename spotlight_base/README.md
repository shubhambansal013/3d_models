...
- **Lock:** three monolithic, revolve-built lugs at 0°/120°/240°. Each lug is 14 mm of rim arc and a 3 mm-wide × 1.2 mm-tall shelf (r 49–52) with no upper rib; the roof overhangs 3 mm past the tip (`roof_capture`) and grips the whole shelf lip on pullout.
...
The roof inner radius equals the shelf lip inner radius, so the full 3 mm lip
(r 49–52) is the catch surface the roof grips on pullout. Because that roof
reaches inside the plate radius, the plate's ceiling-side rim is cut back to
`relief_in` over the three channel arcs (angular span = the roof's full
drop→seat rotation sweep, z 1.2–3 native) so the mount still seats and travels.
`ch_bury` overlaps the roof and back-wall slab past `ch_wall_in` into the solid
wall ring so `mount_plate()` fuses into ONE solid — the slicer detects the skirt
cavity instead of filling it with solid infill (the old 4-solid double wall cost
hours of print time).
...
- The lock grip is 3 mm (`roof_capture`): each lug is a low shelf (no upper rib) so the roof can overhang 3 mm inside the tip and grip the full shelf lip; the plate's ceiling-side rim is relieved over the three channel arcs to make room.
...
14.27 cm³; `mount_fused.brep` = 1 solid, 19.15 cm³) with
`BRepClass3d_SolidClassifier`. Total modeled volume is 33.42 cm³, approximately
41.4 g at PLA density 1.24 g/cm³. Do not use trimesh, pymeshfix, or manifold3d.
